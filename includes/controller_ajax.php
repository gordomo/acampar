...
switch ($_POST['option']) {
    ...
    case "enviar_consulta_index":
        $retorno = array('result' => false, 'mensaje' => 'Error - Imposible enviar mensaje, Intente más tarde | Unable to send message, Try again later');
        if (isset($_POST['nombre']) && $_POST['nombre'] != '' && isset($_POST['email']) && $_POST['email'] != '' && isset($_POST['phone']) && $_POST['phone'] != '' && isset($_POST['consulta']) && $_POST['consulta'] != '') {
            if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
                $retorno = array('result' => false, 'mensaje' => "Email no válido | Invalid Email");
            } else {
                $nombre = $_POST['nombre'];
                $email = $_POST['email'];
                $phone = $_POST['phone'];
            ...
            $retorno = array('result' => false, 'mensaje' => 'Faltan completar datos | Complete missing data');
        }
